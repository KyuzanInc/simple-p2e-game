...
contract PurchaseWithSMP is Script {
    ...
    bytes32 private constant PURCHASE_ORDER_TYPEHASH = keccak256("PurchaseOrder(uint256 purchaseId,address buyer,uint256[] tokenIds,address paymentToken,uint256 amount,uint256 maxSlippageBps,uint256 deadline)");
    ...
    function run() external {
        ...
        for (uint256 i = 0; i < params.purchaseAmount; i++) {
            params.tokenIds[i] = 12345 + i; // Generate sequential tokenIds starting from 12345
        ...
        params.deadline = block.timestamp + 1000000; // 5 minutes from now
        params.purchaseId = uint256(keccak256(abi.encodePacked(block.timestamp, params.user, params.tokenIds.length)));
